  ...
  return (
    <div className={s.actions} data-in-thread={tweet.is_reply}>
      ...
      <div
        ...
        <span onClick={onClickReply} className={`${s.replyText} cursor-pointer`} data-pid={tweet.id_str} data-name={tweet.user.screen_name}>{tweet.replies.length} Comment{tweet.replies.length > 1 ? "s" : ""}</span>
      </div>
      <TweetInfo
        tweet={tweet}
